...
function Fev_ClickButton(buttonId, event) {
    ...
    if (button) {
        ...
        // if (anHRef)
        // {
        SetLastFocusValueOnEnterKey(event);
...
//On EnterKey event, if control is focusable then,  the control id will be assign to lastfocus hidden variable
function SetLastFocusValueOnEnterKey(event) {
    var eventSrc = Fev_GetEventSourceElement(event);
    if (Fev_IsFocusableElement(eventSrc)) {
        var lastFocus = document.getElementById("__LASTFOCUS");
        if (lastFocus != null) {
            lastFocus.value = eventSrc.id;
        }
    } 
}
...
var popupTarget;
function initializePopupPage(sender, src, autopostback, evt) {
...
*  Authors     : Yuenho Leung                                                        *
**************************************************************************************/
function openPopupPage(cssClass) {
