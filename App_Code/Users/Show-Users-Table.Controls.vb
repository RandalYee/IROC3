...
' Show_Users_Table.aspx page.  The Row or RecordControl classes are the 
...
Namespace IROC2.UI.Controls.Show_Users_Table
...
' Base class for the UsersTableControlRow control on the Show_Users_Table page.
...
Public Class BaseUsersTableControlRow
        ...
        Public Overridable Sub SetEnity()
            ...
            ' Set the Enity Literal on the webpage with value from the
            ' DatabaseIROC%dbo.Users database record.

            ' Me.DataSource is the DatabaseIROC%dbo.Users record retrieved from the database.
            ' Me.Enity is the ASP:Literal on the webpage.
            ...
            ' If the Enity is NULL or blank, then use the value specified  
            ' on Properties.
            If Me.Enity.Text Is Nothing _
                OrElse Me.Enity.Text.Trim() = "" Then
                ' Set the value specified on the Properties.
                Me.Enity.Text = "&nbsp;"
            End If
...
#Region "Helper Properties"
        ...
        Public ReadOnly Property Enity() As System.Web.UI.WebControls.Literal
            Get
                Return CType(BaseClasses.Utils.MiscUtils.FindControlRecursively(Me, "Enity"), System.Web.UI.WebControls.Literal)
            End Get
        End Property
...
' Base class for the UsersTableControl control on the Show_Users_Table page.
...
Public Class BaseUsersTableControl
        ...
        Protected Overridable Sub Control_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
            ...
            ' Setup the sorting events.
              ...
              AddHandler Me.EnityLabel.Click, AddressOf EnityLabel_Click
    ...
    Dim recControl As UsersTableControlRow = DirectCast(repItem.FindControl("UsersTableControlRow"), UsersTableControlRow)
                    ...
                    If recControl.Visible AndAlso recControl.IsNewRecord() Then
                        ...
                        If recControl.Agency.Text <> "" Then
                            rec.Parse(recControl.Agency.Text, UsersTable.Agency)
                        End If
                        If recControl.Email.Text <> "" Then
                            rec.Parse(recControl.Email.Text, UsersTable.Email)
                        End If
                        If recControl.Enity.Text <> "" Then
                            rec.Parse(recControl.Enity.Text, UsersTable.Enity)
                        End If
    ...
    Try
        ...
        Public Overridable Sub AgencyLabel_Click(ByVal sender As Object, ByVal args As EventArgs)
            ...
            Dim sd As OrderByItem = Me.CurrentSortOrder.Find(UsersTable.Agency)
            If sd Is Nothing OrElse (Me.CurrentSortOrder.Items IsNot Nothing Andalso Me.CurrentSortOrder.Items.Length > 1) Then
              ...
              Me.CurrentSortOrder.Add(UsersTable.Agency, OrderByItem.OrderDir.Asc)
        ...
        Public Overridable Sub EnityLabel_Click(ByVal sender As Object, ByVal args As EventArgs)
            ' Sorts by Enity when clicked.
              
            ' Get previous sorting state for Enity.
            
            Dim sd As OrderByItem = Me.CurrentSortOrder.Find(UsersTable.Enity)
            If sd Is Nothing OrElse (Me.CurrentSortOrder.Items IsNot Nothing Andalso Me.CurrentSortOrder.Items.Length > 1) Then
                ' First time sort, so add sort order for Enity.
                Me.CurrentSortOrder.Reset()
              ...
              Me.CurrentSortOrder.Add(UsersTable.Enity, OrderByItem.OrderDir.Asc)
            
            Else
                ' Previously sorted by Enity, so just reverse.
                sd.Reverse()
            End If
    ...
    Try
      ...
      ' Enclose all database retrieval/update code within a Transaction boundary
          ...
          Else
             ...
             data.ColumnList.Add(New ExcelColumn(UsersTable.Agency, "Default"))
             data.ColumnList.Add(New ExcelColumn(UsersTable.Enity, "Default"))
    ...
    Try
      ...
      ' Enclose all database retrieval/update code within a Transaction boundary
                ...
                report.SpecificReportFileName = Page.Server.MapPath("Show-Users-Table.UsersPDFButton.report")
                ' report.Title replaces the value tag of page header and footer containing ${ReportTitle}
                report.Title = "Users"
                ' If Show-Users-Table.UsersPDFButton.report specifies a valid report template,
                ...
                ' The 5th parameter represents the relative width of the column   			
                 ...
                 report.AddColumn(UsersTable.Agency.Name, ReportEnum.Align.Left, "${Agency}", ReportEnum.Align.Left, 20)
                 report.AddColumn(UsersTable.Enity.Name, ReportEnum.Align.Left, "${Enity}", ReportEnum.Align.Left, 15)
                    ...
                    If Not (records Is Nothing) AndAlso records.Length > 0 AndAlso whereClause.RunQuery Then
                      For Each record As UsersRecord In records 
                            ...
                            ' The 4th parameters represent the maximum length of the data value being shown
                             ...
                             report.AddData("${Agency}", record.Format(UsersTable.Agency), ReportEnum.Align.Left, 300)
                             report.AddData("${Enity}", record.Format(UsersTable.Enity), ReportEnum.Align.Left, 300)
    ...
    Try
      ...
      ' Enclose all database retrieval/update code within a Transaction boundary
                ...
                report.SpecificReportFileName = Page.Server.MapPath("Show-Users-Table.UsersWordButton.word")
                ' report.Title replaces the value tag of page header and footer containing ${ReportTitle}
                report.Title = "Users"
                ' If Show-Users-Table.UsersWordButton.report specifies a valid report template,
                ' AddColumn method will generate a report template.
                ' Each AddColumn method-call specifies a column
                ' The 1st parameter represents the text of the column header
                ' The 2nd parameter represents the horizontal alignment of the column header
...
#Region "Helper Properties"
        ...
        Public ReadOnly Property EnityLabel() As System.Web.UI.WebControls.LinkButton
            Get
                Return CType(BaseClasses.Utils.MiscUtils.FindControlRecursively(Me, "EnityLabel"), System.Web.UI.WebControls.LinkButton)
            End Get
        End Property
